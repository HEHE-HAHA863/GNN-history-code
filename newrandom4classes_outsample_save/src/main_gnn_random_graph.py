...
def spectral_clustering_adj(A: np.ndarray, k: int, true_labels, normalized: bool = True):
    ...
    return acc_spectral_clustering
...
def train_single_first_period(gnn, optimizer, gen, n_classes, iter):
    # --- 1. 生成图 ---
    W, labels, eigvecs_top, snr, class_sizes = gen.random_sample_otf_single(C = 10, is_training=True, cuda=torch.cuda.is_available())
    ...
    info = ['iter', 'avg loss', 'avg acc', 'edge_density', 'noise', 'model', 'elapsed', 'snr', 'class_sizes']
    out = [iter, loss_value, acc, args.edge_density, args.noise, 'GNN', elapsed_time, snr, str(class_sizes)]

    # print(template1.format(*info))
    # print(template2.format(*out))

    print(template_header.format(*info))
    print(template_row.format(*out))
...
def test_single_first_period(gnn_first_period, gen, n_classes, args, iter, mode='balanced', class_sizes=None):
    ...
    acc_spectral_clustering = spectral_clustering_adj(W, n_classes, true_labels)
    ...
    return loss_test_first, acc_test_first ,acc_spectral_clustering
...
def test_first_period(gnn_first_period, n_classes, gen, iters=args.num_examples_test,
    ...
    for it in range(iters):
        loss_test_first, acc_test_first, acc_spectral_clustering = test_single_first_period(
...
if __name__ == '__main__':
    try:
        ...
        class_sizes_list = [
            [250, 250, 250, 250],  # 完全均衡 ✅
            [200, 250, 250, 300],  # 轻度不均衡 ✅
            [150, 200, 250, 400],  # 中度不均衡 ✅
            [100, 150, 250, 500],  # 明显不均衡 ✅
            [50, 150, 200, 600],  # 极端不均衡 ✅
            [20, 100, 180, 700],  # 极限不均衡 ✅
        ]
